...
import stockNewsLoadMore from './stockNewsLoadMore';
import StockNewsShowMore from './newsArticleShowMore';
import stockNewsArticleDetail from './stockNewsArticleDetail';
...
class NewsAndroidWebview extends Component {
  ...
  componentDidMount() {
    console.log("Props webview " + JSON.stringify(this.props));
  ...
  render() {
    console.log("article url" + this.state.article_url)
    return (
      <SafeAreaView style={[globalStyles.safeviewStyle, this.state.themeStyle.mainView]}>
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', backgroundColor: (this.state.marketStatus == true) ? colors.blue : "#000" }}>
            <TouchableOpacity onPress={() => this.closeWebView()}>
              <View style={{ alignSelf: 'flex-start', padding: 10 }}>
                <Icon name="close" size={30} color={(this.state.marketStatus == true) ? "#FFF" : "#FFF"} />
              </View>
            </TouchableOpacity>
            <View style={{ flex: 1, alignItems: 'flex-end', justifyContent: 'flex-end', flexDirection: 'row' }}>
              <TouchableOpacity onPress={() => { WebViewRef && WebViewRef.reload(); }}>
                <View style={{ padding: 10 }}>
                  <Icon name="refresh" size={30} color={(this.state.marketStatus == true) ? "#FFF" : "#FFF"} />
                </View>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => this.shareMsg()}>
                <View style={{ padding: 10 }}>
                  <Icon name="share-google" size={30} color={(this.state.marketStatus == true) ? "#FFF" : "#FFF"} />
                </View>
              </TouchableOpacity>
            </View>
          </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(NewsAndroidWebview);